...
impl EntityIdAccessor {
	...
	>(&mut self, manager: &EntityManager) -> Option<&Vec<usize>> {
		...
		if needs_update {
			...
			dst.clear();
			for id in src.iter() {
				if manager2.component_exists(*id) && manager3.component_exists(*id) && manager4.component_exists(*id) {
					dst.push(*id);
				}
			}
			self.updated_map.insert(type_id, manager.get_frame());
...
impl EntityManager {
	...
	>(&mut self, entity_id: usize) -> Option<(&mut T1, &mut T2, &mut T3)> {
		...
		if !manager1.component_exists(entity_id) || !manager2.component_exists(entity_id) || !manager3.component_exists(entity_id) {
			return None;
		}
	...
	>(&mut self, entity_id: usize) -> Option<(&mut T1, &mut T2, &mut T3, &mut T4)> {
		...
		if !manager1.component_exists(entity_id) || !manager2.component_exists(entity_id) || !manager3.component_exists(entity_id) || !manager4.component_exists(entity_id) {
			return None;
		}
...
fn cast_manager_mut_unsafe<T: 'static + Component>
	(manager: &Box<dyn ComponentManagerT>) -> &mut ComponentManager<T> {
